...
sophisticated experiments. It is used by thousand of users all around the world
(7,000 downloads per year and 1,000 citations per year of the core Bonsai paper
[Lopes et al., 2015]). Bonsai orchestrates a growing software and hardware
open-source ecosystem, as Open Ephys hardware for electrophysiology, Harp
devices for synchronisation, and the Bonsai.ML package allowing intelligent
experimental control through real-time machine learning.
...
courses, and participating on online forums. Optimally combining these
components is an even harder task. Here we propose to build an AI agent that
will automate the assembly, debugging and optimization of Bonsai workflows from
natural language user prompts from users.
...
will receive as inputs data summaries from Bonsai workflows and provide LLM
...
rules, it will allow to decide on the fate of the experiment based on
...
as relevant for Neuroscience as Image Net has been for vision. Bonsai is the
experimental control software used in Aeon.
...
disciplines. We do not aim to achieve this long-term vision in this project, we
just want to take the first steps on this direction.
